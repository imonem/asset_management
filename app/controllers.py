...
def process_dataframe_for_bulk_update(df):
    ...
    for _, row in df.iterrows():
        ...
        if asset:
            ...
            asset.individual_asset_reference = row.get("individual_asset_reference", asset.individual_asset_reference)
            asset.asset_type = row.get("asset_type", asset.asset_type)
            asset.asset_description = row.get("asset_description", asset.asset_description)
            asset.manufacturer_name = row.get("manufacturer_name", asset.manufacturer_name)
            ...
            asset.contact_point_name = row.get("contact_point_name", asset.contact_point_name)
            asset.contact_point_email = row.get("contact_point_email", asset.contact_point_email)
            asset.contact_point_telephone = row.get("contact_point_telephone", asset.contact_point_telephone)
    ...
    response = {"message": f"{updated_count} records updated successfully", "not_found_barcodes": not_found_barcodes}
    return response, 200
